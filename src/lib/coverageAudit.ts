...
import { algorithmDebugCardCount, algorithmDebugCardsForChapter, algorithmDebugChapterCount, algorithmDebugModeCount, algorithmDebugModes, algorithmDebugRepairStepCount, algorithmDebugTestStepCount } from "@/lib/algorithmDebug";
...
import { readinessCardCount, readinessCardsForChapter, readinessChapterCount, readinessModeCount, readinessModes, readinessPrerequisiteCount, readinessRescueStepCount } from "@/lib/readinessCoach";
...
import { projectCardsForChapter, projectCardCount, projectChapterCount, projectDeliverableCount, projectMilestoneCount, projectModeCount, projectRubricCount, projectSourceCount, projectStudioModes } from "@/lib/projectStudio";
import { evidenceReplayCardCount, evidenceReplayCardsForChapter, evidenceReplayChapterCount, evidenceReplayKindCount, evidenceReplayModeCount, evidenceReplayModes, evidenceReplayReconstructionStepCount } from "@/lib/evidenceReplay";
...
export interface ChapterCoverageRow {
  ...
  readinessModes: number;
  readinessRescueSteps: number;
  readinessPrerequisites: number;
  ...
  projectRubrics: number;
  projectDeliverables: number;
  evidenceReplays: number;
  evidenceReplayModes: number;
  evidenceReplaySteps: number;
  ...
  codeLabs: number;
  codeLabModes: number;
  algorithmDebugClinics: number;
  algorithmDebugModes: number;
  algorithmDebugRepairSteps: number;
  algorithmDebugTestSteps: number;
...
export interface CoverageAudit {
  ...
  totals: {
    ...
    evidenceReplays: number;
    evidenceReplayModes: number;
    evidenceReplayChapters: number;
    evidenceReplaySteps: number;
    evidenceReplayKinds: number;
    lectureTheaters: number;
    lectureTheaterSlides: number;
    lectureTheaterModes: number;
    practiceCards: number;
...
export function buildCoverageAudit(): CoverageAudit {
  ...
  const readinessChecks = readinessCardCount();
  const readinessModeTotal = readinessModeCount();
  const readinessChapters = readinessChapterCount();
  const readinessRescueSteps = readinessRescueStepCount();
  const readinessPrerequisites = readinessPrerequisiteCount();
  ...
  const projects = projectCardCount();
  const projectModeTotal = projectModeCount();
  const projectChapters = projectChapterCount();
  const projectMilestones = projectMilestoneCount();
  ...
  const evidenceReplays = evidenceReplayCardCount();
  const evidenceReplayModeTotal = evidenceReplayModeCount();
  const evidenceReplayChapters = evidenceReplayChapterCount();
  const evidenceReplaySteps = evidenceReplayReconstructionStepCount();
  const evidenceReplayKinds = evidenceReplayKindCount();
  ...
  const algorithmDebugClinics = algorithmDebugCardCount();
  const algorithmDebugModeTotal = algorithmDebugModeCount();
  const algorithmDebugChapters = algorithmDebugChapterCount();
  const algorithmDebugRepairSteps = algorithmDebugRepairStepCount();
  const algorithmDebugTestSteps = algorithmDebugTestStepCount();
  ...
  const totals = {
    ...
    readinessChecks,
    readinessModes: readinessModeTotal,
    readinessChapters,
    readinessRescueSteps,
    ...
    projectSources,
    evidenceReplays,
    evidenceReplayModes: evidenceReplayModeTotal,
    evidenceReplayChapters,
    evidenceReplaySteps,
    evidenceReplayKinds,
    lectureTheaters,
    ...
    codeLabChapters,
    algorithmDebugClinics,
    algorithmDebugModes: algorithmDebugModeTotal,
    algorithmDebugChapters,
    algorithmDebugRepairSteps,
    algorithmDebugTestSteps,
    assumptionClinics,
...
function chapterCoverageRow(chapterNumber: number): ChapterCoverageRow {
  ...
  const readinessChecks = readinessCardsForChapter(chapterNumber);
  const readinessModeTotal = readinessModeCount(chapterNumber);
  const readinessRescueSteps = readinessRescueStepCount(chapterNumber);
  const readinessPrerequisites = readinessPrerequisiteCount(chapterNumber);
  ...
  const projects = projectCardsForChapter(chapterNumber);
  const projectModeTotal = projectModeCount(chapterNumber);
  const projectMilestoneTotal = projectMilestoneCount(chapterNumber);
  const projectRubricTotal = projectRubricCount(chapterNumber);
  const projectDeliverableTotal = projectDeliverableCount(chapterNumber);
  const evidenceReplays = evidenceReplayCardsForChapter(chapterNumber);
  const evidenceReplayModeTotal = evidenceReplayModeCount(chapterNumber);
  const evidenceReplayStepTotal = evidenceReplayReconstructionStepCount(chapterNumber);
  ...
  const algorithmDebugClinics = algorithmDebugCardsForChapter(chapterNumber);
  const algorithmDebugModeTotal = algorithmDebugModeCount(chapterNumber);
  const algorithmDebugRepairSteps = algorithmDebugRepairStepCount(chapterNumber);
  const algorithmDebugTestSteps = algorithmDebugTestStepCount(chapterNumber);
  ...
  const warnings = [
    ...
    readinessChecks.length >= 3 ? "" : "Readiness coach has fewer than three checks.",
    readinessChecks.length * readinessModes.length === readinessModeTotal ? "" : "Readiness coach count does not match mode coverage.",
    readinessRescueSteps >= readinessChecks.length * 5 ? "" : "Readiness coach has fewer than five rescue steps per check.",
    ...
    projects.length >= 3 ? "" : "Project studio has fewer than three chapter projects.",
    projects.length * projectStudioModes.length === projectModeTotal ? "" : "Project studio count does not match mode coverage.",
    projectMilestoneTotal >= projects.length * 5 ? "" : "Project studio has fewer than five build milestones per project.",
    projectRubricTotal >= projects.length * 5 ? "" : "Project studio has fewer than five rubric checks per project.",
    projectDeliverableTotal >= projects.length * 4 ? "" : "Project studio has fewer than four deliverables per project.",
    evidenceReplays.length >= evidence.length ? "" : "Evidence replay studio has fewer cards than figure/example anchors.",
    evidenceReplays.length * evidenceReplayModes.length === evidenceReplayModeTotal ? "" : "Evidence replay studio count does not match mode coverage.",
    evidenceReplayStepTotal >= evidenceReplays.length * 4 ? "" : "Evidence replay studio has fewer than four reconstruction steps per card.",
    ...
    algorithmDebugClinics.length >= algorithms.length ? "" : "Algorithm debugging clinic has fewer cards than algorithm cards.",
    algorithmDebugClinics.length * algorithmDebugModes.length === algorithmDebugModeTotal ? "" : "Algorithm debugging clinic count does not match mode coverage.",
    algorithmDebugRepairSteps >= algorithmDebugClinics.length * 4 ? "" : "Algorithm debugging clinic has fewer than four repair steps per card.",
    algorithmDebugTestSteps >= algorithmDebugClinics.length * 4 ? "" : "Algorithm debugging clinic has fewer than four test fixture steps per card.",
  ...
  return {
    ...
    projectModes: projectModeTotal,
    projectMilestones: projectMilestoneTotal,
    projectRubrics: projectRubricTotal,
    projectDeliverables: projectDeliverableTotal,
    evidenceReplays: evidenceReplays.length,
    evidenceReplayModes: evidenceReplayModeTotal,
    evidenceReplaySteps: evidenceReplayStepTotal,
    lectureTheaters: 1,
    ...
    codeLabModes: codeLabModeTotal,
    algorithmDebugClinics: algorithmDebugClinics.length,
    algorithmDebugModes: algorithmDebugModeTotal,
    algorithmDebugRepairSteps,
    algorithmDebugTestSteps,
...
function requirementProofs(totals: CoverageAudit["totals"]): RequirementProof[] {
  return [
    ...
    {
      label: "Readiness coach",
      status: totals.readinessChecks >= totals.chapters * 3 && totals.readinessModes >= totals.readinessChecks * readinessModes.length && totals.readinessChapters === totals.chapters && totals.readinessRescueSteps >= totals.readinessChecks * 5 ? "complete" : "warning",
      evidence: `${totals.readinessChecks} readiness checks cover ${totals.readinessChapters}/${totals.chapters} chapters with ${totals.readinessModes} diagnose/bridge/visual/technical/exit modes, ${totals.readinessRescueSteps} rescue steps, and ${totals.readinessPrerequisites} prerequisite handles.`,
      easy: "Readers can check whether they are ready for a chapter, repair the missing prerequisite, draw it, and pass an exit question before continuing.",
      technical: "readinessCoach.ts derives entry, math, and method diagnostics from zeroKnowledge, foundation, math, concept, formula, algorithm, and chapter metadata; ReadinessCoach renders the interactive mode console on /readiness, home, /book, and chapter routes.",
    },
    {
    ...
    {
      label: "Project studios",
      status: totals.projects >= totals.chapters * 3 && totals.projectModes >= totals.projects * projectStudioModes.length && totals.projectChapters === totals.chapters && totals.projectMilestones >= totals.projects * 5 && totals.projectRubrics >= totals.projects * 5 && totals.projectDeliverables >= totals.projects * 4 ? "complete" : "warning",
      evidence: `${totals.projects} project cards cover ${totals.projectChapters}/${totals.chapters} chapters with ${totals.projectModes} brief/build/experiment/rubric/extend modes, ${totals.projectMilestones} build milestones, ${totals.projectRubrics} rubric checks, ${totals.projectDeliverables} deliverables, and ${totals.projectSources} source types.`,
      easy: "Readers can turn each chapter into buildable evidence: an artifact, an experiment, a self-grade, and a transfer extension.",
      technical: "projectStudio.ts derives case-build, code-experiment, and capstone project cards from case studies, simulators, code labs, method comparisons, exercise solutions, and chapter exams; ProjectStudio renders brief, build, experiment, rubric, and extend modes on /projects, home, /book, and chapter routes.",
    },
    {
      label: "Evidence replay studios",
      status: totals.evidenceReplays >= totals.evidenceAnchors && totals.evidenceReplayModes >= totals.evidenceReplays * evidenceReplayModes.length && totals.evidenceReplayChapters === totals.chapters && totals.evidenceReplaySteps >= totals.evidenceReplays * 4 ? "complete" : "warning",
      evidence: `${totals.evidenceReplays} evidence replay cards cover ${totals.evidenceReplayChapters}/${totals.chapters} chapters with ${totals.evidenceReplayModes} read/reconstruct/technical/pitfall/transfer modes, ${totals.evidenceReplaySteps} reconstruction steps, and ${totals.evidenceReplayKinds} source kinds.`,
      easy: "Readers can study figures, tables, and named examples as replayable lessons instead of static references.",
      technical: "evidenceReplay.ts derives mode-switching replay cards from evidenceGuideItems; EvidenceReplayStudio renders read, reconstruct, technical, pitfall, and transfer modes on /evidence, home, /book, and chapter routes without copying source artwork or prose.",
    },
    {
    ...
    },
    {
      ...
      technical: "bookIndex.ts aggregates chapter stories, zero-primer rungs, foundation dictionary terms, math rescue objects, lecture slides, section mastery cards, symbol cards, active recall, concept cards, examples, misconception repairs, simulators, implementation code labs, algorithm debugging clinics, assumption clinics, manuscripts, section lessons, synthesis/dependency gates, source audits, algorithms, mastery notes, formulas, evidence anchors, exercise coaches, exercise solution cards, project cards, and method comparison cards into serialized search entries consumed by BookSearch.",
    },
    {
      ...
      technical: "learningGraph.ts derives typed graph nodes and edges from original chapter, concept, section-mastery, formula, algorithm, case-study, project-studio, method-comparison, worked-example, practice, exercise-solution, simulator, code-lab, algorithm-debug-clinic, assumption-clinic, and dependency modules; LearningGraphExplorer renders the interactive SVG graph on /graph, home, and chapter routes.",
    },
    {
    ...
    {
      label: "Algorithm debugging clinics",
      status: totals.algorithmDebugClinics >= totals.algorithms && totals.algorithmDebugModes >= totals.algorithmDebugClinics * algorithmDebugModes.length && totals.algorithmDebugChapters === totals.chapters && totals.algorithmDebugRepairSteps >= totals.algorithmDebugClinics * 4 && totals.algorithmDebugTestSteps >= totals.algorithmDebugClinics * 4 ? "complete" : "warning",
      evidence: `${totals.algorithmDebugClinics} algorithm debugging clinics cover ${totals.algorithmDebugChapters}/${totals.chapters} chapters with ${totals.algorithmDebugModes} symptom/diagnose/repair/test/transfer modes, ${totals.algorithmDebugRepairSteps} repair steps, and ${totals.algorithmDebugTestSteps} test-fixture checks.`,
      easy: "Readers can diagnose a failed method before tuning: name the symptom, isolate the first broken cause, repair one thing, and retest on a tiny world.",
      technical: "algorithmDebug.ts joins algorithmCatalog failure modes with codeLab test plans and assumptionClinic diagnostics/repairs; AlgorithmDebugClinic renders symptom, diagnose, repair, test, and transfer modes on /debug, home, /book, and chapter routes.",
    },
    {
