...
function buildBookIndexEntries(): BookIndexEntry[] {
  ...
  for (const chapter of chapters) {
    ...
    mathCards.forEach((card) => add({
      ...chapterBase,
      id: card.id,
      title: `Math rescue: ${card.object}`,
      layer: "math rescue",
      route: `/chapters/${chapter.n}#math-rescue`,
      summary: `${card.intuition} Picture: ${card.boardPicture}`,
    ...
    examCards.forEach((card) => add({
      ...chapterBase,
      id: card.id,
      title: `Exam: ${card.title}`,
      layer: "chapter exam",
      route: `/chapters/${chapter.n}#exam`,
      summary: `${card.prompt} Diagnostic: ${card.diagnostic}`,
      technical: `Plan: ${card.plan.join(" ")} Solution: ${card.solution} Rubric: ${card.rubric.join(" ")} Transfer: ${card.transfer}`,
      tags: [card.kind, ...card.tags, `Chapter ${chapter.n}`],
      weight: 84,
    }));
