...
const visualInteractionRows = [
  {
    ...
    coverage: "All overview sections render an animated concept graphic through the shared Section wrapper or a dedicated interactive lecture console, including the readiness coach, foundation dictionary studio, math rescue studio, visual story studio, analogy bridge studio, Socratic tutor studio, case study studio, project studio, evidence replay studio, guided lecture theater, section mastery studio, whole-book search console, interactive learning graph, contextual symbol decoder, implementation code lab, algorithm debugging clinic, assumption guarantee clinic, method comparison studio, proof intuition lab, chapter exam studio, and chapter simulator lab.",
  ...
  },
  {
    ...
    interaction: "Each concept map exposes Sense, Target, Update, and Act states; each readiness coach switches diagnose/bridge/visual/technical/exit modes; each foundation dictionary switches meaning/picture/technical/trap/teach-back modes; each math rescue studio switches intuition/picture/notation/chapter-use/self-check modes; each chapter search console filters query/layer/chapter state; each learning graph switches chapter/node/view state; each symbol decoder switches symbol/formula/pitfall/check state; each code lab switches plain/code/invariant/test/debug modes; each algorithm debugging clinic switches symptom/diagnose/repair/test/transfer modes; each assumption clinic switches plain/assumption/guarantee/failure/repair modes; each method comparison studio switches choose/axis/tradeoff/failure/bridge modes; each proof lab switches plain/claim/proof/equation/stress modes; each exam studio switches prompt/plan/solution/rubric/transfer modes; each section mastery studio switches prompt/hint/answer/technical/transfer modes; each starter ladder has four learning modes; each lecture theater has five slides and five modes; each practice coach has five reveal modes; each concept microscope has five lecture modes; each worked example studio has five worked modes; each misconception clinic has five repair modes; each simulator has three sliders and four live readouts; each blackboard has four click-through stages and a technical toggle; each section reader has six mode controls; dense cards add animated micro-glyphs.",
  },
  {
    surface: "Linear book interactive labs",
    coverage: "The /book reader renders every chapter's compact readiness coach, foundation dictionary, math rescue studio, visual story studio, analogy bridge studio, Socratic tutor studio, case study studio, project studio, evidence replay studio, lecture theater, simulator lab, blackboard, symbol decoder, implementation code lab, algorithm debugging clinic, assumption guarantee clinic, method comparison studio, proof lab, chapter exam studio, section mastery studio, and formula lecturer inline, so the continuous web-book path has guided lecture slides, live tradeoff controls, visual chapter models, notation walkthroughs, code scaffolds, validity contracts, proof sketches, chapter exams, section-by-section mastery checks, and equation walkthroughs.",
    ...
    interaction: "Readers choose readiness checks, switch prerequisite repair modes, choose foundation terms, switch dictionary modes, choose math objects, switch rescue modes, choose story scenes, switch story lecture-camera modes, choose analogy bridges, switch analogy modes, choose tutor cards, switch question/hint/board/technical/try modes, choose case studies, switch scene/walkthrough/technical/debug/transfer modes, choose project cards, switch brief/build/experiment/rubric/extend modes, choose evidence anchors, switch read/reconstruct/technical/pitfall/transfer modes, switch lecture slides/modes, move simulator sliders, click stage pills, toggle technical panels, choose symbol cards, choose implementation cards, choose algorithm debugging clinics, choose assumption clinics, choose method comparisons, choose proof cards, choose exam cards, choose section mastery cards, choose formula cards, and switch equation lecture modes while staying inside the linear reading flow.",
  },
  {
    ...
    <main className="min-h-screen bg-bg text-ink">
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-8 lg:px-10">
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_460px] lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-3xl text-lg leading-relaxed text-muted">This page is generated from the current repository data. It checks whether the site has a linear /book reader, a whole-book search index, a foundation dictionary studio, a math rescue studio, a visual story studio, an analogy bridge studio, a Socratic tutor studio, a case study studio, a project studio, an evidence replay studio, an interactive learning graph, a contextual symbol decoder, an implementation code lab, an algorithm debugging clinic, a readiness coach, an assumption guarantee clinic, a method comparison studio, a proof intuition lab, a chapter exam studio, a section mastery studio, exercise solution studio, every chapter has a standalone from-scratch lecture route with a chapter-local search console, chapter learning map, notation console, interactive blackboard, section-level textbook manuscript, and guided section lecture controls, and every algorithm card has the required easy, technical, derivation, profile, dossier, and worked-example layers.</p>
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-line bg-white">
                ...
                <Stat value={String(audit.totals.readinessChecks)} label="readiness checks" />
                <Stat value={String(audit.totals.readinessModes)} label="readiness modes" />
                <Stat value={String(audit.totals.readinessRescueSteps)} label="readiness rescues" />
                <Stat value={String(audit.totals.readinessPrerequisites)} label="ready prereqs" />
                <Stat value={String(audit.totals.foundationDictionaryCards)} label="foundation terms" />
                <Stat value={String(audit.totals.foundationDictionaryModes)} label="foundation modes" />
                <Stat value={String(audit.totals.mathRescueCards)} label="math rescue" />
                <Stat value={String(audit.totals.mathRescueModes)} label="math modes" />
                <Stat value={String(audit.totals.visualStoryCards)} label="story scenes" />
                <Stat value={String(audit.totals.visualStoryModes)} label="story modes" />
                <Stat value={String(audit.totals.visualStoryActors)} label="story actors" />
                <Stat value={String(audit.totals.visualStoryProps)} label="story props" />
                <Stat value={String(audit.totals.analogyCards)} label="analogies" />
                <Stat value={String(audit.totals.analogyModes)} label="analogy modes" />
                <Stat value={String(audit.totals.analogyMappings)} label="analogy maps" />
                <Stat value={String(audit.totals.socraticTutorCards)} label="tutor cards" />
                <Stat value={String(audit.totals.socraticTutorModes)} label="tutor modes" />
                <Stat value={String(audit.totals.socraticTutorTurns)} label="tutor turns" />
                <Stat value={String(audit.totals.socraticTutorBoardSteps)} label="tutor board steps" />
                <Stat value={String(audit.totals.caseStudies)} label="case studies" />
                <Stat value={String(audit.totals.caseStudyModes)} label="case modes" />
                <Stat value={String(audit.totals.caseStudyBoardFrames)} label="case board frames" />
                <Stat value={String(audit.totals.caseStudySuccessCriteria)} label="case success checks" />
                <Stat value={String(audit.totals.projects)} label="projects" />
                <Stat value={String(audit.totals.projectModes)} label="project modes" />
                <Stat value={String(audit.totals.projectMilestones)} label="project milestones" />
                <Stat value={String(audit.totals.projectRubrics)} label="project rubrics" />
                <Stat value={String(audit.totals.projectDeliverables)} label="project deliverables" />
                <Stat value={String(audit.totals.evidenceReplays)} label="evidence replays" />
                <Stat value={String(audit.totals.evidenceReplayModes)} label="replay modes" />
                <Stat value={String(audit.totals.evidenceReplaySteps)} label="replay steps" />
                ...
                <Stat value={String(audit.totals.algorithmDebugClinics)} label="debug clinics" />
                <Stat value={String(audit.totals.algorithmDebugModes)} label="debug modes" />
                <Stat value={String(audit.totals.algorithmDebugRepairSteps)} label="debug repairs" />
                <Stat value={String(audit.totals.algorithmDebugTestSteps)} label="debug tests" />
      ...
      <div className="mx-auto grid max-w-[1280px] gap-12 px-6 py-12 lg:px-10">
        ...
        <section id="chapter-matrix" className="scroll-mt-24">
          <SectionTitle eyebrow="02 - Chapter coverage matrix" title="Every chapter route and every major study layer in one audit table." lead="Open any chapter to inspect the rendered detail layers: chapter search index, readiness coach, foundation dictionary studio, math rescue studio, visual story studio, analogy bridge studio, Socratic tutor studio, case study studio, project studio, evidence replay studio, interactive learning graph, contextual symbol decoder, implementation code lab, algorithm debugging clinic, assumption guarantee clinic, method comparison studio, proof intuition lab, chapter exam studio, section mastery studio, exercise solution studio, guided lecture theater, concept microscope, worked example studio, misconception clinic, chapter simulator lab, manuscript, interactive blackboard, interactive section lecturer, section-level textbook prose, standalone lecture, synthesis, dependencies, source audit, algorithms, deep dives, mastery notes, formulas, anchors, and exercises." />
...
function ChapterCoverageCard({ chapter }: { chapter: ChapterCoverageRow }) {
  return (
    <article className="rounded-xl border border-line bg-panel p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-[repeat(79,minmax(0,1fr))]">
        ...
        <Metric label="readiness" value={chapter.readinessChecks} />
        <Metric label="ready modes" value={chapter.readinessModes} />
        <Metric label="ready rescues" value={chapter.readinessRescueSteps} />
        <Metric label="ready prereqs" value={chapter.readinessPrerequisites} />
        <Metric label="foundations" value={chapter.foundationDictionaryCards} />
        <Metric label="foundation modes" value={chapter.foundationDictionaryModes} />
        <Metric label="math rescue" value={chapter.mathRescueCards} />
        <Metric label="math modes" value={chapter.mathRescueModes} />
        <Metric label="stories" value={chapter.visualStoryCards} />
        <Metric label="story modes" value={chapter.visualStoryModes} />
        <Metric label="analogies" value={chapter.analogyCards} />
        <Metric label="analogy modes" value={chapter.analogyModes} />
        <Metric label="tutor" value={chapter.socraticTutorCards} />
        <Metric label="tutor modes" value={chapter.socraticTutorModes} />
        <Metric label="tutor turns" value={chapter.socraticTutorTurns} />
        <Metric label="tutor board" value={chapter.socraticTutorBoardSteps} />
        <Metric label="cases" value={chapter.caseStudies} />
        <Metric label="case modes" value={chapter.caseStudyModes} />
        <Metric label="case frames" value={chapter.caseStudyBoardFrames} />
        <Metric label="case checks" value={chapter.caseStudySuccessCriteria} />
        <Metric label="projects" value={chapter.projects} />
        <Metric label="project modes" value={chapter.projectModes} />
        <Metric label="project milestones" value={chapter.projectMilestones} />
        <Metric label="project rubrics" value={chapter.projectRubrics} />
        <Metric label="project deliverables" value={chapter.projectDeliverables} />
        <Metric label="evidence replays" value={chapter.evidenceReplays} />
        <Metric label="replay modes" value={chapter.evidenceReplayModes} />
        <Metric label="replay steps" value={chapter.evidenceReplaySteps} />
        ...
        <Metric label="debug clinics" value={chapter.algorithmDebugClinics} />
        <Metric label="debug modes" value={chapter.algorithmDebugModes} />
        <Metric label="debug repairs" value={chapter.algorithmDebugRepairSteps} />
        <Metric label="debug tests" value={chapter.algorithmDebugTestSteps} />
        ...
        <Metric label="section mastery" value={chapter.sectionMasteryCards} />
        <Metric label="section modes" value={chapter.sectionMasteryModes} />
...
function glyphVariantForLabel(label: string): "loop" | "bars" | "tree" | "target" | "formula" | "check" {
  ...
  if (/warning|complete|check|status|exam|rubric|section mastery/i.test(label)) return "check";
  return "loop";
}
