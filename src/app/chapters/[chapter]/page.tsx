...
import { exerciseSolutionCardsForChapter, exerciseSolutionModeCount, type ExerciseSolutionCard } from "@/lib/exerciseSolutionStudio";
...
import { readinessCardsForChapter, readinessModeCount, type ReadinessCoachCard } from "@/lib/readinessCoach";
...
import { caseStudiesForChapter, caseStudyModeCount, type ChapterCaseStudy } from "@/lib/caseStudies";
import { projectCardsForChapter, projectModeCount, type ChapterProjectCard } from "@/lib/projectStudio";
import { evidenceReplayCardsForChapter, evidenceReplayModeCount, type EvidenceReplayCard } from "@/lib/evidenceReplay";
...
import { algorithmDebugCardsForChapter, algorithmDebugModeCount, type AlgorithmDebugCard } from "@/lib/algorithmDebug";
...
export default async function ChapterPage({ params }: { params: Params }) {
  ...
  const exerciseSolutions = exerciseSolutionCardsForChapter(item.n);
  const exerciseSolutionModes = exerciseSolutionModeCount(item.n);
  ...
  const readinessCards = readinessCardsForChapter(item.n);
  const readinessModes = readinessModeCount(item.n);
  const foundationCards = foundationDictionaryCardsForChapter(item.n);
  ...
  const caseStudies = caseStudiesForChapter(item.n);
  const caseModes = caseStudyModeCount(item.n);
  const projects = projectCardsForChapter(item.n);
  const projectModes = projectModeCount(item.n);
  const evidenceReplays = evidenceReplayCardsForChapter(item.n);
  const evidenceReplayModes = evidenceReplayModeCount(item.n);
  ...
  const debugCards = algorithmDebugCardsForChapter(item.n);
  const debugModes = algorithmDebugModeCount(item.n);
  ...
  return (
    <main className="min-h-screen bg-bg text-ink">
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_420px] lg:items-end">
            ...
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-line bg-white">
                ...
                <Stat value={String(readinessCards.length)} label="readiness checks" />
                <Stat value={String(readinessModes)} label="readiness modes" />
                <Stat value={String(foundationCards.length)} label="foundation terms" />
                ...
                <Stat value={String(caseStudies.length)} label="case studies" />
                <Stat value={String(caseModes)} label="case modes" />
                <Stat value={String(projects.length)} label="projects" />
                <Stat value={String(projectModes)} label="project modes" />
                <Stat value={String(evidenceReplays.length)} label="evidence replays" />
                <Stat value={String(evidenceReplayModes)} label="replay modes" />
                ...
                <Stat value={String(debugCards.length)} label="debug clinics" />
                <Stat value={String(debugModes)} label="debug modes" />
                ...
                <Stat value={String(exerciseSolutions.length)} label="exercise solutions" />
                <Stat value={String(exerciseSolutionModes)} label="solution modes" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto grid max-w-[1280px] gap-12 px-6 py-12 lg:px-10">
        ...
        <ChapterIndex n={item.n} counts={{ algorithms: algorithms.length, starterRungs: starter.rungs.length, starterModes, readinessCards: readinessCards.length, readinessModes, foundationCards: foundationCards.length, foundationModes, mathCards: mathCards.length, mathModes, storyCards: storyCards.length, storyModes, analogyCards: analogyCards.length, analogyModes, tutorCards: tutorCards.length, tutorModes, caseStudies: caseStudies.length, caseModes, projects: projects.length, projectModes, evidenceReplays: evidenceReplays.length, evidenceReplayModes, theaterSlides, theaterModes, practiceCards: practiceCards.length, practiceModes, conceptCards: conceptCards.length, conceptModes, workedExamples: workedExamples.length, workedExampleModes, misconceptionCards: misconceptionCards.length, misconceptionModes, manuscriptSections: manuscript.sections.length, blackboardStages: blackboard.stages.length, sectionLessons: sectionLessons.length, lectureBeats: lecture.beats.length, sections: deep?.sectionDetails.length ?? 0, formulas: formulas.length, formulaModes, evidence: evidence.length, exercises: exercises.length, simulatorControls, simulatorReadouts, sourceAudits: sourceAudits.length, searchEntries: searchEntryTotal, graphNodes, graphEdges, symbolCards: symbolCards.length, symbolModes, codeCards: codeCards.length, codeModes, debugCards: debugCards.length, debugModes, assumptionCards: assumptionCards.length, assumptionModes, compareCards: compareCards.length, compareModes, proofCards: proofCards.length, proofModes, examCards: examCards.length, examModes, sectionMasteryCards: sectionMasteryCards.length, sectionMasteryModes, exerciseSolutions: exerciseSolutions.length, exerciseSolutionModes }} />
        ...
        <ChapterReadinessBlock chapter={item} cards={readinessCards} readinessModes={readinessModes} />
        ...
        <ChapterCaseStudyBlock chapter={item} cases={caseStudies} caseModes={caseModes} />

        <ChapterProjectBlock chapter={item} projects={projects} projectModes={projectModes} />

        <ChapterEvidenceReplayBlock chapter={item} cards={evidenceReplays} evidenceReplayModes={evidenceReplayModes} />
        ...
        <ChapterDebugBlock chapter={item} cards={debugCards} debugModes={debugModes} />
        ...
        <ChapterExerciseSolutionBlock chapter={item} cards={exerciseSolutions} exerciseSolutionModes={exerciseSolutionModes} />
...
function ChapterIndex({ n, counts }: { n: number; counts: { algorithms: number; starterRungs: number; starterModes: number; readinessCards: number; readinessModes: number; foundationCards: number; foundationModes: number; mathCards: number; mathModes: number; storyCards: number; storyModes: number; analogyCards: number; analogyModes: number; tutorCards: number; tutorModes: number; caseStudies: number; caseModes: number; projects: number; projectModes: number; evidenceReplays: number; evidenceReplayModes: number; theaterSlides: number; theaterModes: number; practiceCards: number; practiceModes: number; conceptCards: number; conceptModes: number; workedExamples: number; workedExampleModes: number; misconceptionCards: number; misconceptionModes: number; simulatorControls: number; simulatorReadouts: number; manuscriptSections: number; blackboardStages: number; sectionLessons: number; sectionMasteryCards: number; sectionMasteryModes: number; lectureBeats: number; sections: number; formulas: number; formulaModes: number; evidence: number; exercises: number; sourceAudits: number; searchEntries: number; graphNodes: number; graphEdges: number; symbolCards: number; symbolModes: number; codeCards: number; codeModes: number; debugCards: number; debugModes: number; assumptionCards: number; assumptionModes: number; compareCards: number; compareModes: number; proofCards: number; proofModes: number; examCards: number; examModes: number; exerciseSolutions: number; exerciseSolutionModes: number } }) {
  const items = [
    ["search", `${counts.searchEntries} search entries`],
    ["readiness", `${counts.readinessCards} readiness checks · ${counts.readinessModes} modes`],
    ...
    ["case-studies", `${counts.caseStudies} case studies · ${counts.caseModes} modes`],
    ["projects", `${counts.projects} projects · ${counts.projectModes} modes`],
    ["evidence-replay", `${counts.evidenceReplays} evidence replays · ${counts.evidenceReplayModes} modes`],
    ...
    ["debug", `${counts.debugCards} debug clinics · ${counts.debugModes} modes`],
...
function ChapterReadinessBlock({ chapter, cards, readinessModes }: { chapter: (typeof chapters)[number]; cards: ReadinessCoachCard[]; readinessModes: number }) {
  return (
    <section id="readiness" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00b - Readiness coach"
        title="Repair the prerequisite before the chapter becomes technical."
        lead="Every readiness card asks whether the reader can explain, draw, technically name, and exit-check the entry loop, calculation, or method trace before moving deeper."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-cyan/30 bg-cyan/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} readiness promise:</span> {cards.length} checks become {readinessModes} diagnose, bridge, visual, technical, and exit modes so readers who know nothing can repair the missing prerequisite before continuing.</p>
        </div>
        <ReadinessCoach cards={cards} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}

function ChapterFoundationDictionaryBlock({ chapter, cards, foundationModes }: { chapter: (typeof chapters)[number]; cards: FoundationDictionaryCard[]; foundationModes: number }) {
  return (
    <section id="foundations" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00b - Foundation dictionary studio"
        title="Define the chapter's language before the dense lecture uses it."
        lead="Every foundation card gives a plain meaning, board picture, technical role, common trap, and teach-back check so no RL vocabulary is assumed."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-cyan/30 bg-cyan/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} foundation promise:</span> {cards.length} terms become {foundationModes} meaning, picture, technical, trap, and teach-back modes before the chapter moves into notation and algorithms.</p>
        </div>
        <FoundationDictionaryStudio cards={cards} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}
...
function ChapterMathRescueBlock({ chapter, cards, mathModes }: { chapter: (typeof chapters)[number]; cards: MathRescueCard[]; mathModes: number }) {
  return (
    <section id="math-rescue" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00c - Math rescue studio"
        title="Translate the chapter's calculations before manipulating the notation."
        lead="Every rescue card starts with why the mathematical object exists, draws it on the board, names every symbol, places it in the chapter argument, and ends with a no-notes check."
      />
      <div className="mt-6 grid gap-4">
...
function ChapterAnalogyBlock({ chapter, cards, analogyModes }: { chapter: (typeof chapters)[number]; cards: AnalogyCard[]; analogyModes: number }) {
  return (
    <section id="analogies" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00e - Analogy bridge studio"
        title="Map familiar examples to exact RL language without trusting the analogy too far."
        lead="Every analogy starts with an everyday doorway, maps pieces into RL objects, translates the mapping technically, marks its limits, and ends with a transfer check."
      />
      <div className="mt-6 grid gap-4">
...
function ChapterCaseStudyBlock({ chapter, cases, caseModes }: { chapter: (typeof chapters)[number]; cases: ChapterCaseStudy[]; caseModes: number }) {
  return (
    <section id="case-studies" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00g - Case study studio"
        title="Run the chapter as complete scenarios, not isolated facts."
        lead="Each case joins a concrete world, a board-frame walkthrough, a technical translation, a debugging probe, and a transfer challenge so the learner can see the full RL loop working."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-lime/30 bg-lime/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} case-study promise:</span> {cases.length} case studies become {caseModes} scene, walkthrough, technical, debug, and transfer modes so chapter ideas are tested inside complete learner-world-feedback scenarios.</p>
        </div>
        <CaseStudyStudio cases={cases} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}

function ChapterProjectBlock({ chapter, projects, projectModes }: { chapter: (typeof chapters)[number]; projects: ChapterProjectCard[]; projectModes: number }) {
  return (
    <section id="projects" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00h - Project studio"
        title="Turn the chapter into buildable evidence."
        lead="Each project asks for a visible artifact, a tiny experiment, a rubric-based self-grade, and an extension so the chapter becomes something you can build and defend."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-lime/30 bg-lime/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} project promise:</span> {projects.length} projects become {projectModes} brief, build, experiment, rubric, and extension modes so explanations turn into artifacts, experiments, and transfer evidence.</p>
        </div>
        <ProjectStudio projects={projects} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}
...
function ChapterEvidenceReplayBlock({ chapter, cards, evidenceReplayModes }: { chapter: (typeof chapters)[number]; cards: EvidenceReplayCard[]; evidenceReplayModes: number }) {
  return (
    <section id="evidence-replay" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00i - Evidence replay studio"
        title="Replay the chapter figures, tables, and examples as mechanisms."
        lead="Each evidence card teaches how to read an anchor, reconstruct it from memory, translate it technically, avoid the common misread, and transfer the same mechanism to a small new RL world."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-cyan/30 bg-cyan/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} evidence promise:</span> {cards.length} anchors become {evidenceReplayModes} read, reconstruct, technical, pitfall, and transfer modes so figures and examples become reusable reasoning tools.</p>
        </div>
        <EvidenceReplayStudio cards={cards} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}
...
function ChapterDebugBlock({ chapter, cards, debugModes }: { chapter: (typeof chapters)[number]; cards: AlgorithmDebugCard[]; debugModes: number }) {
  return (
    <section id="debug" className="scroll-mt-24">
      <SectionTitle
        eyebrow="00e - Algorithm debugging clinic"
        title="Diagnose failed methods before tuning more knobs."
        lead="This layer teaches the practical failure loop: symptom, diagnostic trace, one-cause repair, tiny retest, and transfer to a new task."
      />
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-orange/30 bg-orange/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} debug promise:</span> {cards.length} debug clinics become {debugModes} symptom, diagnose, repair, test, and transfer modes tied back to chapter algorithms.</p>
        </div>
        <AlgorithmDebugClinic cards={cards} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact />
      </div>
    </section>
  );
}
...
function ChapterExerciseSolutionBlock({ chapter, cards, exerciseSolutionModes }: { chapter: (typeof chapters)[number]; cards: ExerciseSolutionCard[]; exerciseSolutionModes: number }) {
  return (
    <section id="exercise-solutions" className="scroll-mt-24">
      ...
      <div className="mt-6 grid gap-4">
        <div className="rounded-xl border border-orange/30 bg-orange/[0.06] p-4">
          <p className="text-sm leading-relaxed text-muted"><span className="font-medium text-ink">Chapter {chapter.n} exercise promise:</span> {cards.length} exercise solution cards expose {exerciseSolutionModes} attempt, hint, solution, debug, and extension modes tied back to the chapter exercise coach.</p>
        </div>
        {cards.length ? <ExerciseSolutionStudio cards={cards} contextTitle={`Chapter ${chapter.n}: ${chapter.title}`} compact /> : <p className="rounded-lg border border-line bg-white p-4 text-sm leading-relaxed text-muted">This chapter has no numbered exercise anchor in the current exercise-coach data, so use the active recall and chapter exam layers for practice.</p>}
      </div>
    </section>
  );
}
